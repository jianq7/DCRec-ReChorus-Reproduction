	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			if len(prediction.shape)==2: # only for ranking tasks
				...
				# 2. 计算总损失（推荐损失 + 对比损失）
			if hasattr(model, 'calculate_contrastive_loss') and model.cl_weight>0:
				# 如果是DCRec模型：计算推荐损失 + 对比损失
				rec_loss = model.loss(out_dict)  # 父类的推荐损失（单参数）
				cl_loss = model.calculate_contrastive_loss(batch)  # 对比损失
				loss = rec_loss + model.cl_weight * cl_loss  # 总损失
			else:
				# 兼容其他模型：仅计算推荐损失
